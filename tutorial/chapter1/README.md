...
- http://127.0.0.1:8018/docs にブラウザでアクセスして、APIの動作を確かめてみましょう
...
Webフレームワークは様々な紆余曲折を経て、どのAPIにもほぼほぼ共通で現れる記述をなるべく隠蔽するような進化を辿っています。  
そのため、初めてWebフレームワークに触れ方の中には「これだけ?」「なんか動いているけど正直良くわからない」というような感想を抱いてしまう方もいます。
そこで、この章では昔ながらのwebを実装して、webの仕組を理解しながら、レガシーな実装の何が問題だったかを見ていきましょう。

## Requestを受け取ってResponseを返却する

APIとはHTTPリクエストを受け取ってHTTPレスポンスを返すいわゆる関数です。  
FastAPIでもリクエストオブジェクト( `Request` )を受け取って、レスポンスオブジェクト( `Response` )を返却するという形式で記述ができます。  
...
GET, POSTメソッドで受け取ったリクエストをそのままjson形式でレスポンスするAPIを。実装します。
...
では、ここから、昔ながらのWebアプリのどこがまずいのかを考えていきましょう。
...
HTMLを返却するAPIにはもう一つ問題があります。それは処理とデザインが密結合してしまうということです。  

1つ目の問題点はAPIの再利用ができないことです。  
似たような処理は共通化したいところですが、処理にデザインがデザインがくっついていると再利用できません。  

2つ目の問題は考慮事項が増えることです。  
つまり「処理だけを変更したいのにデザインも考慮して修正する必要がある」、もしくは「デザインだけを変更したいのに処理も考慮して修正する必要がある」ということです。  
こうなってしまうと、リファクタリングや機能追加の際に考慮事項が増え開発スピードが低下します。加えてテストが困難で意図しないバグも発生しやすくなります。

# モダンなAPIへ

レガシーなWebアプリのダメな部分を見てきました。  
FastAPI本来の書き方で再実装して、ダメな部分がどのように改善するのかを見ていきましょう。  
